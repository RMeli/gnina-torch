...
for fold in range(1, nfolds + 1):
    ...
    ids_cnnscore = df.groupby(by=["system"])["pose_prob"].idxmax()
    ...
    pearson["CNNscore"].append(
        stats.pearsonr(
            df.loc[ids_cnnscore, "affinity_pred"], df.loc[ids_cnnscore, "affinity_exp"]
        )[0]
    )
...
order = ["best", "CNNscore", "CNNaffinity", "worst"]
...
plt.savefig("results/pearson-box.pdf")
...
plt.savefig("results/rmse-box.pdf")
